...
app.post('/add-event', upload.single("event_image"), async (req, res) => {
  try {
    ...
    const receiptPrintValue = req.body.receipt_print ? 1 : 0;
    const queueValue = req.body.queue ? 1 : 0;
...
app.post('/add-transaction', async (req, res) => {
  ...
  db.query(eventSql, [transactionData.event_id], async (err, eventResults) => {
    ...
    const receiptPrintEnabled = eventResults[0].receipt_print === 1;
    const queueEnabled = eventResults[0].queue === 1;
    ...
    db.query(queueNumberSql, [transactionData.event_selected_date, transactionData.event_selected_showtime], (err, queueNumberResult) => {
      ...
      const sql = `
        INSERT INTO transactions (
          ...
          event_selected_date, event_selected_showtime, queue_number, knet_amount
        ) VALUES ?
      `;
      ...
      db.query(sql, [values], async (err, result) => {
        ...
        if (transactionData.payment_method === 'Cash') {
          ...
          db.query(updateSql, [transactionData.cash_received, transactionData.event_id], (err, result) => {
            ...
            // Generate receipts or send QR code based on receipt_print field
            if (receiptPrintEnabled) {
              generateReceiptsForActors(validActors, transactionData, transactionId, queueNumber, res, queueEnabled);
            } else {
              sendQRCodeViaWhatsApp(transactionData, transactionId, res);
            }
          });
        } else {
          // Generate receipts or send QR code based on receipt_print field
...
// Route to fetch transaction details by transaction_id
app.get('/get-transaction-details', (req, res) => {
  const transactionId = req.query.transaction_id;

  const sql = `
    SELECT 
      transaction_id, event_name, event_selected_date, event_selected_showtime, 
...
async function sendQRCodeViaWhatsApp(transactionData, transactionId, res) {
  try {
    // Generate QR code
    const qrCodeData = `${transactionId}`;
    const qrCodeImagePath = path.join(__dirname, 'public', 'qrcodes', `${transactionId}.png`);

    // Ensure the qrcodes directory exists
    if (!fs.existsSync(path.dirname(qrCodeImagePath))) {
      fs.mkdirSync(path.dirname(qrCodeImagePath), { recursive: true });
    }
    ...
    const message = encodeURIComponent(
      ...
      `📅 Event: ${transactionData.event_name}\n` +
      `💰 Total Amount: ${transactionData.total_amount.toFixed(2)} KWD\n` +
      `📸 Scan the QR code below for transaction details:\n\n` +
      `QR Code: ${qrCodeImageUrl}`
    );

    // Replace with the actual customer phone number (including country code)
    const phoneNumber = transactionData.customer_phone; // Example: "96512345678" (Kuwait)
    const whatsappLink = `https://wa.me/${phoneNumber}?text=${message}`;

    // Send the WhatsApp link as a response
    res.send({
      success: true,
    ...
    res.status(500).send({ success: false, message: 'Failed to generate WhatsApp link' });
  }
}

async function generateReceiptsForActors(validActors, transactionData, transactionId, queueNumber, res, queueEnabled) {
  try {
...
app.post('/archive-event', (req, res) => {
  ...
  function generateReceiptHTML(transactionData, queueEnabled) {
    return `
      <!DOCTYPE html>
      <html lang="en">
      <head>
...
app.get('/edit-event-form', (req, res) => {
  ...
  app.post('/generate-receipts', async (req, res) => {
    const { transactionData , queueEnabled} = req.body;
    ...
    try {
      ...
      const receiptHTML = generateReceiptHTML(transactionData, queueEnabled);
  ...
  app.post('/refund-transaction', async (req, res) => {
    ...
    try {
      ...
      const transactionSql = `
        SELECT t.*, e.camira_price, e.mobile_price
        FROM transactions t
      ...
      db.query(transactionSql, [transaction_id], (err, transactionResults) => {
        ...
        const camiraRefund = transaction.camira_photos * transaction.camira_price;
        const mobileRefund = transaction.mobile_photos * transaction.mobile_price;
        const totalRefund = camiraRefund + mobileRefund;
  
        // Update the transaction with the refund amount and set cash_received to 0
        const updateSql = `
          UPDATE transactions
        ...
        db.query(updateSql, [totalRefund, transaction_id], (err, updateResult) => {
          ...
          res.send({ success: true, message: 'Refund processed successfully!', totalRefund });
        });
      });
    } catch (error) {
